...
export class MSSCrawler {
  ...
  async crawlNoticesList(lastKnownNumber: string | null = null): Promise<NoticeResult> {
    try {
      ...
      console.log(`[MSSCrawler] 증분 크롤링 시작 (마지막 번호: ${lastKnownNumber || '없음'})`);
      
      // 페이지별로 크롤링 (최대 3페이지)
      const MAX_PAGES = 3;
      while (shouldContinue && pageIndex <= MAX_PAGES) {
        ...
        if (shouldContinue && pageIndex < MAX_PAGES) {
          pageIndex++;
          // 페이지 간 딜레이 (서버 부하 방지)
          ...
          if (pageIndex >= MAX_PAGES) {
            console.log(`[MSSCrawler] 최대 페이지(${MAX_PAGES}) 도달. 크롤링 종료.`);
          }
          break;
        }
      }
